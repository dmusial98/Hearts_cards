    ...
    public class TxtFileLogReader : IGameReader
    {
        ...
        public async Task<GameHistory> GetGameHistoryFromFileAsync()
        {
            ...
            var startGameLog = listLines
                .Where(s => s[..3] == Consts.GAME_STARTED_CODE_CONST.PadRight(3, ' '))
                .ToArray();
            ...
            string[] roundStartedLogs = listLines
                .Where(s => s[..3] == Consts.ROUND_STARTED_CODE_CONST.PadRight(3, ' '))
                .OrderBy(s => s)
                .ToArray();
            ...
            foreach (string roundStartedLog in roundStartedLogs)
            {
                ...
                var pointsInRound = listLines
                    .Where(s => s[..3] == Consts.PLAYERS_POINTS_IN_ROUND_CODE_CONST.PadRight(3, ' '))
                    .OrderBy(s => s)
                    .Take(Consts.TRICKS_NUMBER_IN_ROUND_CONST * Consts.PLAYERS_NUMBER_CONST)
                    .ToList();

                //for every trick in round
                for (int i = 0; i < Consts.TRICKS_NUMBER_IN_ROUND_CONST; i++)
                {
                ...
                var pointsAfterRoundLog = listLines
                    .Where(s => s[..3] == Consts.PLAYERS_POINTS_AFTER_ROUND_CODE_CONST.PadRight(3, ' '))
                    .OrderBy(s => s)
                    .Take(Consts.PLAYERS_NUMBER_CONST)
                    .ToArray();
